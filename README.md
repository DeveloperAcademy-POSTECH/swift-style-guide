...
- 데이터를 가져오는 함수의 경우, `get` 사용을 지양하고 `request`, `fetch`을 적절하게 사용해주세요.
    - `request` : 에러가 발생하거나, 실패할 수 있는 비동기 작업에 사용합니다. 예를들어 http 통신을 통해 값을 요청하는 경우가 이에 해당합니다
    - `fetch` : 요청이 실패하지 않고 결과를 바로 반환할 때 사용합니다. 예를들어 data를 찾고자 하는 모든 행위를 할 때가 이에 해당합니다.
    - **Good ✅**
        ```swift
